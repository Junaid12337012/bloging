...
const CategoryPage: React.FC = () => {
  ...
  const { categories, publishedPosts: posts } = useData();
  ...
  if (!category) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/categories" className="mt-4 inline-block text-primary-600 hover:underline">
  ...
  const relatedTags = filteredPosts.reduce((acc: Tag[], post) => {
  ...
  return (
    ...
    >
      ...
      {relatedTags.length > 0 && (
          <div className="mb-12">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-4">Related Topics</h2>
            <div className="flex flex-wrap gap-3">
                {relatedTags.map(tag => (
                    <Link key={tag.id} to={`/tag/${tag.id}`} className="px-4 py-2 text-sm font-medium bg-slate-100 dark:bg-slate-800 border border-transparent rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                        {tag.name}
                    </Link>
                ))}
            </div>
      ...
      {currentPosts.length > 0 ? (
        ...
        <div className="text-center py-16 border-t border-slate-200 dark:border-slate-700">
            <p className="text-slate-500 dark:text-slate-400 text-lg">There are no stories in this category yet.</p>
        </div>
      )}
    </motion.div>
  );
